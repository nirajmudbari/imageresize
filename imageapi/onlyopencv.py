...
def _is_big_enough(image, imagesize):
    if (image.size[1] >= imagesize[0]) and (image.size[0] >= imagesize[1]):
        return 1
    else:
        return 0
...
def getimageandreturnbase64(pilimage, size):
    ...
    if resized.mode in ('RGBA', 'LA'):
        ...
        img = background
        img.save('bbbb.jpg', quality=100)
        time.sleep(1)
        newbase64 = image_to_base64()
        removefile()
        return newbase64
    else:
        resized.save('bbbb.jpg', quality=95)
        time.sleep(2)
        newbase64 = image_to_base64()
        removefile()
        return newbase64
...
def getBase64ResizeAndReturnBase64(base64):
    try:
        ...
        elif (decodedimg.size[1] < 300 and decodedimg.size[0] < 600):
